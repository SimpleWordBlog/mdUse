...
1. 字数严格控制在{self.length_entry.get()}字以内，简洁
2. 不要使用markdown语法和格式（不要使用标题、列表符号、强调符号等）
3. 使用连贯的、流畅的叙述性文本
4. 只提取文档中最核心、最重要的信息
5. 使用客观、简洁的语言风格
6. 摘要应该是一段完整的文字，不要分段,不要有空格,不要有空行，始终紧凑前面的内容位置。
7. 可以参考这个例子: 该文档介绍了如何在 Vitepress 中使用选项卡（tabs）插件。示例代码展示了如何使用 vitepress-plugin-tabs插件创建npm和yarn两种安装方式的选项卡。选项卡中包含各自的安装命令，方便用户根据需要选择相应的安装方法。

...
class MarkdownSummarizer:
    def __init__(self):
        ...
        self.window.geometry("1024x768")  # 增大窗口初始大小
    ...
    def setup_ui(self):
        ...
        self.api_key_entry = self.create_labeled_entry(
            settings_frame, 
            "API密钥:", 
            self.config.get('api_key', os.getenv('GROQ_API_KEY', '')),
            password=True
        )
        
        self.model_entry = self.create_labeled_entry(
            settings_frame,
            "模型:",
            self.config.get('model', 'llama-3.3-70b-versatile')
        )
        
        self.rate_entry = self.create_labeled_entry(
            settings_frame,
            "每分钟请求数限制:",
            str(self.config.get('rate_limit', 30))
        )
        ...
        self.length_entry = self.create_labeled_entry(
            summary_frame,
            "摘要长度（字符数）:",
    ...
    def process_markdown_file(self, file_path):
        ...
        try:
            ...
            client = Groq(api_key=self.api_key_entry.get())
            prompt = f"请为以下markdown内容生成一个简洁的中文摘要，字数限制在{self.length_entry.get()}字以内：\n\n{content}"
    ...
    def save_config(self, show_message=False):
        ...
        config = {
            ...
            'model': self.model_entry.get(),
            'rate_limit': int(self.rate_entry.get()),
        ...
        self.rate_limiter = RateLimiter(int(self.rate_entry.get()))
